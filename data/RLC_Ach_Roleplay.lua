...
local function RegisterRoleplayAchievements()
  ...
  if RLC_Achievements.AddTitle then
    for _, title in ipairs(RP_TITLE_REWARDS) do
      RLC_Achievements:AddTitle(title)
    end
  end
end
...
local function CheckRoleplayInteractionAchievements()
  ...
  for _, ach in ipairs(RP_INTERACTION_ACHIEVEMENTS) do
    if MatchesZone(ach, zoneName, subZoneName)
      ...
      RLC_Achievements:AwardAchievement(ach.id)
    end
  end
end
...
rpFrame:SetScript("OnEvent", function()
  if event == "ADDON_LOADED" and arg1 == "RedLobsterCultAchievements" then
    ...
    if RLC_Achievements and RLC_Achievements.CheckAchievementMetaAchievements then
