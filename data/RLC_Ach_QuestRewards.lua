...
local function AwardQuestRewardUse(itemName)
  if not RLC_Achievements or not RLC_Achievements.AwardAchievement then return end
...
local function ScanQuestRewardEquipment(silent)
  if not RLC_Achievements or not RLC_Achievements.AwardAchievement or not GetInventoryItemLink then return end
  local awarded = {}
  for slot = 1, 19 do
    ...
    if achId and not awarded[achId] then
      awarded[achId] = true
      RLC_Achievements:AwardAchievement(achId, silent)
    end
  end
end
...
local function InstallQuestRewardUseHooks()
  ...
  if UseContainerItem then
    ...
    UseContainerItem = function(bag, slot, onSelf)
      ...
      if RLC_Achievements and RLC_Achievements.initialized and itemName then
...
rewardFrame:SetScript("OnEvent", function()
  ...
  elseif event == "UNIT_INVENTORY_CHANGED" and arg1 == "player" and RLC_Achievements and RLC_Achievements.initialized then
