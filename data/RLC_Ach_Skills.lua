...
-- Adapted from KeijinAchievementMonitor for RLC.
...
local function RegisterSkillAchievements()
  if not RLC_Achievements or type(RLC_Achievements.AddAchievement) ~= "function" then return end
  for _, prof in ipairs(PROFESSIONS) do
    for _, step in ipairs(PROF_STEPS) do
      local achId = "prof_"..string.lower(prof.id).."_"..step.value
      RLC_Achievements:AddAchievement(achId, {
        id=achId,
...
local function CheckSkillMilestones(silent)
  if not RLC_Achievements or type(RLC_Achievements.AwardAchievement) ~= "function" then return end
    ...
    if skillName and not isHeader and skillRank then
      ...
      if profKey then
        for _, threshold in ipairs(PROF_MILESTONES) do
          if skillRank >= threshold then
            RLC_Achievements:AwardAchievement("prof_"..profKey.."_"..threshold, silent)
          end
        end
      end
      ...
      if weapKey then
        for _, threshold in ipairs(WEAPON_MILESTONES) do
          if skillRank >= threshold then
            RLC_Achievements:AwardAchievement("weapon_"..weapKey.."_"..threshold, silent)
          end
        end
      end
    end
