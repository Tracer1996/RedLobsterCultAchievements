...
local function CheckChain(me, chain, silent)
  if RLC_Achievements:HasAchievement(me, chain.id) then return end
  if not RLC_AchievementsDB or not RLC_AchievementsDB.completedQuests then return end
  local completed = RLC_AchievementsDB.completedQuests[me]
...
local function EnsureQuestCompletion(me, questName, silent)
  ...
  if not RLC_AchievementsDB.completedQuests[me] then RLC_AchievementsDB.completedQuests[me] = {} end
...
if RLC_Achievements then
  RLC_Achievements.EnsureQuestCompletion = EnsureQuestCompletion
end
...
if RLC_Achievements then
  RLC_Achievements.RecordQuestCompletion = RecordQuestCompletion
end
...
local function CheckRaidAttunementBackfill(silent)
  local me = RLC_Achievements and RLC_Achievements.ShortName and RLC_Achievements.ShortName(UnitName("player"))
...
local function RegisterQuestChainAchievements()
  RLC_Achievements.NormalizeQuestStepKey = NormalizeQuestKey
  for _, chain in ipairs(QUEST_CHAINS) do
    RLC_Achievements:AddAchievement(chain.id, {
      id          = chain.id,
      name        = chain.name,
      desc        = chain.desc,
      category    = "Quests",
    ...
    if chain.title and RLC_Achievements.AddTitle then
...
questFrame:SetScript("OnEvent", function()
  if event == "ADDON_LOADED" and arg1 == "RedLobsterCultAchievements" then
    ...
    local me = RLC_Achievements and RLC_Achievements.ShortName and
               RLC_Achievements.ShortName(UnitName("player"))
    if me then
      for _, chain in ipairs(QUEST_CHAINS) do
