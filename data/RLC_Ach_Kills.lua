-- RLC_Ach_Kills.lua
-- Kill achievements adapted from KeijinAchievementMonitor for RLC.
...
local function RegisterKillAchievements()
  for _, ach in ipairs(GENERIC_KILLS) do
    RLC_Achievements:AddAchievement(ach.id, {
  ...
  for _, ach in ipairs(NAMED_KILLS) do
    ...
    if RLC_Achievements.RegisterProgressDef then
      RLC_Achievements:RegisterProgressDef(ach.id, {counter="nkills_"..ach.id, goal=ach.value})
    end
  end
...
killRegFrame:SetScript("OnEvent", function()
  if event == "ADDON_LOADED" and arg1 == "RedLobsterCultAchievements" then
    if RLC_Achievements and RLC_Achievements.AddAchievement then
      RegisterKillAchievements()
    end
    -- Backlog: award critter achievements already reached from prior sessions
    if not (RLC_Achievements and RLC_Achievements.ShortName) then return end
    local me = RLC_Achievements.ShortName(UnitName("player"))
    if me and RLC_AchievementsDB and RLC_AchievementsDB.progressCounters then
      local pc = RLC_AchievementsDB.progressCounters[me]
      if pc then
        for _, ach in ipairs(NAMED_KILLS) do
          local total = pc["nkills_"..ach.id] or 0
          if total >= ach.value then
            RLC_Achievements:AwardAchievement(ach.id, true)
          end
        end
      end
    end
...
killFrame:SetScript("OnEvent", function()
  ...
  if event == "CHAT_MSG_COMBAT_HOSTILE_DEATH" then
    ...
    if not targetName then
      ...
      if slainTarget and slainByName and RLC_Achievements and RLC_Achievements.ShortName then
        local myName = RLC_Achievements.ShortName(UnitName("player"))
        local killerShort = RLC_Achievements.ShortName(slainByName)
        local inGroup = false
        if RLC_Achievements.IsPartyOrSelf then
          inGroup = RLC_Achievements.IsPartyOrSelf(slainByName)
    ...
    if targetName and RLC_Achievements and RLC_Achievements.ShortName then
      local me = RLC_Achievements.ShortName(UnitName("player"))
      if me then
        local total = RLC_Achievements.IncrCounter(me, "genericKills")
        for i = 1, table.getn(GENERIC_MILESTONES) do
          local m = GENERIC_MILESTONES[i]
          if total >= m.value then
            RLC_Achievements:AwardAchievement(m.id)
          end
        end
      end
    end
    ...
    if achId and RLC_Achievements then
      local goal = NAMED_KILL_GOALS[achId]
      if goal then
        -- Critter: track cumulative kills
        local me = RLC_Achievements.ShortName(UnitName("player"))
        if me then
          local total = RLC_Achievements.IncrCounter(me, "nkills_"..achId)
          if total >= goal then
            RLC_Achievements:AwardAchievement(achId)
          end
        end
      else
